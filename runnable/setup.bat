...
echo 	Write-Host "Is:" -nonewline; Write-Host " $java " -f yellow -nonewline; $path = Read-Host -Prompt "path to SDK? [y/n]" >> setup.ps1
echo 	if($path -eq "y"){ >> setup.ps1
echo 		$path = $java; >> setup.ps1
echo 	} else { >> setup.ps1
echo 		$path = Read-Host -Prompt 'Path to SDK root directory' >> setup.ps1
echo 	} >> setup.ps1
echo } else { >> setup.ps1
echo 	$path = Read-Host -Prompt 'Path to SDK root directory' >> setup.ps1
echo } >> setup.ps1
echo Write-Host "" >> setup.ps1
echo [System.Environment]::SetEnvironmentVariable("JAVA_JDK", $path, [System.EnvironmentVariableTarget]::User) 
echo if($?){ >> setup.ps1
...
echo 	Remove-Item $MyINvocation.InvocationName >> setup.ps1
echo } else { >> setup.ps1
